...
class ServerConfig:
    ...
    max_order_notional_jpy: float = 1_500_000.0  # For JP market
...
def load_config(env_file: str | None = None) -> ServerConfig:
    ...
    region_id = os.environ.get("WEBULL_REGION_ID", "us").lower()
    environment = os.environ.get("WEBULL_ENVIRONMENT", "uat").lower()
    ...
    return ServerConfig(
        ...
        max_order_notional_jpy=max_order_notional_jpy,
...
def validate_config(config: ServerConfig) -> None:
    ...
    if config.environment not in {"uat", "prod"}:
        raise ConfigError(
            "WEBULL_ENVIRONMENT must be one of: uat, prod. "
