...
_JP_QUOTE_URL = "webull.co.jp/pricing"
# ---------------------------------------------------------------------------
# ConfigError
# ---------------------------------------------------------------------------
...
class TestHandleServerException:
    ...
    def test_market_data_403_returns_jp_hint(self):
        exc = ServerException(code="FORBIDDEN", msg=_NO_ACCESS_MSG, http_status=403)
        result = handle_sdk_exception(exc, "get_stock_snapshot", region_id="jp")
        assert _JP_QUOTE_URL in result
...
class TestMarketDataSubscriptionHints:
    ...
    def test_jp_hint_contains_url(self):
        hint = _get_market_data_hint("jp")
        assert _JP_QUOTE_URL in hint
        assert "developer.webull.co.jp" in hint

    def test_fallback_contains_all_regions(self):
